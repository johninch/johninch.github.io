...
ts中用小写字母开头的类型代表字面量, 大写的是ES内置的，用来表示通过new实例化的数据：
```js
// 字面量
const n:number = 123;
const s:string = '456';
...
对于联合类型，在还不确定类型的时候，只能访问所有联合类型公有的属性或方法，如果访问其中某种类型所特有的属性或方法时，就需要将其断言为所属联合类型中的某种类型：
```ts
// 写法一
<类型>值

// 写法二：在 tsx 语法（React 的 jsx 语法的 ts 版）中必须用这一种。
值 as 类型
```
